...
<div class="bg-white shadow rounded-lg p-6 max-w-2xl mx-auto">
    ...
    <form action="{{ route('proyek_invoice.update', $invoice->id) }}" method="POST" class="space-y-4">
        @csrf @method('PUT')

        <div>
            <label class="block text-gray-700 mb-1">Nomor Invoice</label>
            <input type="text" name="nomor_invoice" value="{{ $invoice->nomor_invoice }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div>
            <label class="block text-gray-700 mb-1">Judul Invoice</label>
            <input type="text" name="judul_invoice" value="{{ $invoice->judul_invoice }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div>
            <label class="block text-gray-700 mb-1">Jumlah</label>
            <input type="number" name="jumlah" step="0.01" value="{{ $invoice->jumlah }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div>
            <label class="block text-gray-700 mb-1">Tanggal Invoice</label>
            <input type="date" name="tanggal_invoice" value="{{ $invoice->tanggal_invoice }}" class="w-full border rounded px-3 py-2" required>
        </div>

        <div>
            <label class="block text-gray-700 mb-1">Keterangan</label>
            <textarea name="keterangan" class="w-full border rounded px-3 py-2">{{ $invoice->keterangan }}</textarea>
        </div>
